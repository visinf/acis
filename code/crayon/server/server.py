...
parser.add_argument("--logdir", type=str, default='/tmp/tensorboard/{}',
          help="Tensorboard log directory")
parser.add_argument("--port", type=int, default=8889,
          help="Port where to listen for incoming datas")
parser.add_argument("--tb-port", type=int, default=8888,
...
tensorboard_folder = path.join(cli_args.logdir, "{}")
...
def tb_request(query_type, run=None, tag=None, safe=True):
  request_url = "http://localhost:{}".format(cli_args.tb_port) + "/data/{}"
  if run and tag:
    request_url += "?run={}&tag={}"
...
def get_version():
  ...
  if not json.loads(req_res)["logdir"] == tensorboard_folder[:-3]:
    print(json.loads(req_res)["logdir"])
    print(tensorboard_folder[:-3])
