     ...
     while idx <= size and threads:acceptsjob() do
        ...
        threads:addjob(
           function(indices)
              ...
              local index, inputs, paths, origs, fgs, targets
              for i, idx in ipairs(indices:totable()) do
                 local sample_orig = _G.dataset:get(idx)
                 local origImg = sample_orig.input:clone()
                 ...
                 table.insert(origs, origImg)
                 if sample.fg then table.insert(fgs, sample.fg) end
                 if sample.target then table.insert(targets, sample.target) end
              end
              local out = {}
              out.index = index[1]
              out.input = inputs
